...
class dataLoadController
{
    ...
    public function bellload($officer_id)
    {
        // return $table;
        // die();
        $sql = $this->conn->prepare("SELECT COUNT(*) AS bell FROM notification WHERE status = '0' AND to_officer_id = '${officer_id}'");
        $sql->execute();
    ...
    {
        if ($officer_id == null) {
            $query = "SELECT SUM(deposit) AS total_dps_collection, 
                (SELECT SUM(total_loan) FROM loan_profiles WHERE status = '1' AND MONTH(created_at) = MONTH(CURRENT_DATE())) AS total_loan, 
                (SELECT SUM(loan) FROM loan_collections WHERE status = '1' AND MONTH(created_at_date) = MONTH(CURRENT_DATE())) AS total_loan_collection,
                (SELECT SUM(deposit) FROM saving_collections WHERE period_id != '25' AND status = '1' AND MONTH(created_at_date) = MONTH(CURRENT_DATE())) AS Total_savings_collection 
                FROM saving_collections WHERE period_id = '25' AND status = '1' AND MONTH(created_at_date) = MONTH(CURRENT_DATE())";
        } else {
            $query = "SELECT SUM(deposit) AS total_dps_collection, 
                (SELECT SUM(total_loan) FROM loan_profiles WHERE reg_officer_id = '${officer_id}' AND status = '1' AND MONTH(created_at) = MONTH(CURRENT_DATE())) AS total_loan, 
                (SELECT SUM(loan) FROM loan_collections WHERE officers_id = '${officer_id}' AND status = '1' AND MONTH(created_at_date) = MONTH(CURRENT_DATE())) AS total_loan_collection,
                (SELECT SUM(deposit) FROM saving_collections WHERE officers_id = '${officer_id}' AND period_id != '25' AND status = '1' AND MONTH(created_at_date) = MONTH(CURRENT_DATE())) AS Total_savings_collection 
