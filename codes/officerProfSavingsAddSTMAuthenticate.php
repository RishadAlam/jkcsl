...
$query = "SELECT s.book, s.deposit_installment, s.interest, s.total_with_interest, s.created_at, c.name, f.field_name, cn.center_name, p.period_name  FROM saving_profiles AS s INNER JOIN client_registers AS c ON s.client_id = c.client_id INNER JOIN feilds AS f ON f.feild_id = s.field_id INNER JOIN centers AS cn ON cn.center_id = s.center_id INNER JOIN periods AS p ON p.period_id = s.period_id WHERE s.status = '1' AND s.reg_officer_id = '${officer_id}' AND s.created_at BETWEEN '${from_date}' AND '${end_date}' ORDER BY s.saving_profiles_id DESC";
