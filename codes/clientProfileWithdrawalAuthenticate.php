...
$query = "SELECT sw.withdrawal, sw.balance_remaining, sw.created_at AS date, sw.expression, u.name FROM saving_withdrawals AS sw INNER JOIN users AS u ON sw.officers_id = u.id WHERE sw.savings_prof_id ='${savingsID}' AND sw.status='1' AND sw.created_at BETWEEN '${from_date}' AND '${end_date}'";
